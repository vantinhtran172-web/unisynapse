...
export default function Home() {
  ...
  const { activeTab, setActiveTab, user, tasks, documents, ledger, loading, error } = useAppState();
  ...
  if (!mounted) return <main className="loading-screen"><div className="loading-mark">UH</div><span>Đang chuẩn bị không gian tri thức…</span></main>;
  return <main className="client-shell"><div className="client-glow client-glow-one" /><div className="client-glow client-glow-two" /><Navbar /><div className="client-container">
    ...
    {activeView === "overview" && <section className="client-hero"><div className="hero-copy"><div className="hero-kicker"><span className="kicker-line" /> LỚP TRI THỨC MỞ <span className="kicker-line" /></div><h1>Học tập cùng nhau.<br /><em>Xác minh mọi điều.</em></h1><p>UniHackFest turns focused student contribution into a living academic network. Label data, share knowledge, and build an AI tutor you can trust.</p><div className="hero-actions"><button className="primary-action" onClick={() => goToTab("challenges")}>Khám phá nhiệm vụ <span>↗</span></button><button className="secondary-action" onClick={() => goToTab("learning")}>Xem lộ trình học</button></div><div className="hero-trust"><span>Được cộng đồng ngày càng lớn tin dùng</span><div className="avatar-stack"><i>AL</i><i>MK</i><i>TN</i><i>+</i></div><b>Đóng góp và kiểm định tri thức</b></div></div><div className="hero-orbit"><div className="orbit-ring orbit-ring-a" /><div className="orbit-ring orbit-ring-b" /><div className="orbit-center"><span>UH</span><small>LIVE<br />NETWORK</small></div><div className="orbit-node orbit-node-a">AI</div><div className="orbit-node orbit-node-b">◎</div><div className="orbit-node orbit-node-c">↗</div></div></section>}
    <section className="metrics-grid"><Metric label="Đóng góp của bạn" value={`${ledger.filter((entry) => entry.delta > 0).length}`} detail={loading ? "Đang tải dữ liệu thật…" : error ? "Lỗi kết nối backend" : "Từ sổ cái thật"} tone="cyan" /><Metric label="Điểm tri thức" value={`${user?.reputation ?? 0}`} detail="Reputation từ database" tone="violet" /><Metric label="Số dư UniPoints" value={`${user?.unipoints ?? 0}`} detail="Số dư hiện tại" tone="amber" /><Metric label="Tài liệu đã nộp" value={`${documents.length}`} detail="Tài liệu từ API" tone="emerald" /></section>
    ...
    <footer className="client-footer"><div className="footer-brand"><span className="brand-badge">UH</span><strong>UniHackFest</strong><span>Trí tuệ học thuật mở.</span></div><div className="footer-links"><button onClick={() => goToTab("overview")}>Trang chủ</button><button onClick={() => goToTab("learning")}>Học tập</button><button onClick={() => goToTab("tutor")}>AI Tutor</button><button onClick={() => goToTab("ledger")}>Proof</button></div><span className="footer-copy">© 2026 · Solana Devnet</span></footer>
